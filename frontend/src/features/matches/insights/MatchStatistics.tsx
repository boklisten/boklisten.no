...
export default function MatchStatistics({ roundId }: { roundId: string }) {
  ...
  const verdictDonut = [
    { name: "Som planlagt", value: data.handoverVerdicts.asPlanned, color: "teal.6" },
    {
      name: "Mottatt fra en annen enn forventet",
      value: data.handoverVerdicts.fromUnexpectedSender,
      color: "yellow.6",
    },
    {
      name: "Levert til en annen enn forventet",
      value: data.handoverVerdicts.toUnexpectedRecipient,
      color: "orange.6",
    },
  ];
  const settledHalves = verdictDonut.reduce((sum, slice) => sum + slice.value, 0);
  ...
  return (
    <Stack gap="lg">
      ...
      >
        <DonutWithLegend centerLabel={settledHalves.toLocaleString("nb-NO")} data={verdictDonut} />
      </ChartCard>
