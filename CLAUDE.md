...
`boklisten.no` is a monorepo for a library and book management service for upper secondary schools. It unifies what were previously separate `bl-web` and `bl-admin` projects into a single customer-facing site with integrated administration (the admin section keeps the name "bl-admin"; the legacy app itself was shut down in September 2026).
